angular.module('myApp').controller('ChequeDishonorController', function ($scope, $window, $http) {

    $('#pageTitle').text("--- Cheque Dishonor");
    ...
    $scope.loadCheqDishonorList = function () {
        $http.get('/Transaction/ChequeDishonor/getCheqDishonorList/').
          success(function (data) {
              if (data.success) {
                  $scope.rowCollection = data.cheqDishonorList;
    ...
    $scope.dishonorCheque = function () {
        if ($scope.selected.length == 0) {
            ...
            $http({
                method: 'POST',
                url: '/Transaction/ChequeDishonor/dishonorCheque',
                data: $scope.selected
            }).success(function (data) {
                if (data.success) {
                    toastr.success("Cleared Successfully");
                    $scope.loadCheqDishonorList();
                    $scope.selected = [];
                    $scope.totalSelectedAmount = 0;
                }
                else {
