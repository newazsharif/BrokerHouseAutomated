angular.module('myApp').controller('ImportCDBLController', function ($scope, $window, $http, $filter, globalvalueservice) {

    $('#pageTitle').text("--- Import CDBL Transaction Files");
    ...
    $scope.loadCDBLFileList = function () {
        ...
        $http.get('/CDBL/ImportCDBL/getCDBLFileList/').
    ...
    $scope.importCDBLFiles = function () {
        if ($scope.selected.length == 0) {
            toastr.error("Please select files to import!!!!");
        }
        else {
            ...
            $http({
                method: 'POST',
                url: '/CDBL/ImportCDBL/importCDBLTransFiles',
