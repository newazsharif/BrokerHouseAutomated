...
///Reason: Cheque Clear Controller
///=============================================================

namespace Escrow.BOAS.Web.Areas.Transaction.Controllers
{
    public class ChequeClearController : Controller
    {
        ...
        public ChequeClearController()
            : this(new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext())))
        {

        }

        public ChequeClearController(UserManager<ApplicationUser> userManager)
        {
        ...
        public JsonResult getChequeClrList()
        {
            try
            {
                ...
                var cheqclrList = fundReceiveFactory.GetAll().Where(a => a.approve_by != null && a.deposit_by != null && a.clear_by == null && a.dishonor_by == null && a.tblConstantObjectValue.display_value != "CS")
                    .Select(s => new
                    {
                        s.id,
                ...
                var jsonResult = Json(new { cheqclrList = cheqclrList, success = true }, JsonRequestBehavior.AllowGet);
        ...
        [CheckUserSessionAttribute]
        [HttpPost]
        public JsonResult clearCheque(string[] selected)
        {
            try
            {
                decimal membership_id = SessionManger.BrokerOfLoggedInUser(Session).membership_id;
                ...
                for (int i = 0; i < selected.Length; i++)
                {
                    ...
                    {
                        obj.clear_by = User.Identity.GetUserId();
                        obj.clear_date = DateTimeConfig.FullDateUKtoDateKey(SessionManger.LoggedInTime(Session).ToString("dd/MM/yyyy"));
                        obj.is_dirty = 1;
